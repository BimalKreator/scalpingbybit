...
def _run_backtest_long_push_scalping(
    df: pd.DataFrame,
    params: dict,
    *,
    trade_amount_usd: float = 100.0,
    ...
    """
    Signal on closed bar i (last row of slice); fill at open of bar i+1; exit that bar at SL/TP touch or close.
    Uses ``long_push_scalping.evaluate`` (LONG-only, absolute SL/TP).
    """
    from strategies.long_push_scalping import DEFAULT_PARAMS as LPS_DEFAULTS
    from strategies.long_push_scalping import evaluate as lps_evaluate

    p = {**LPS_DEFAULTS, **(params or {})}
    tam = float(p.get("tradeCapitalUsd") or trade_amount_usd)
    lev = float(p.get("leverage") or leverage)
    ...
    if df2.empty or len(df2) < 3:
        print(f"[backtest long_push_scalping] skip: len={len(df2)} need>=3")
        return empty
    ...
    while i < len(df2) - 1:
        ...
        try:
            sl_f = float(sl_price) if sl_price is not None else float("nan")
            tp_f = float(tp_price) if tp_price is not None else float("nan")
        except (TypeError, ValueError):
            i += 1
            continue
        if not math.isfinite(sl_f) or not math.isfinite(tp_f) or sl_f <= 0 or tp_f <= 0:
            i += 1
            continue
        ...
        exit_reason = "time"
        if l2 <= sl_f:
            exit_price = float(sl_f)
            exit_reason = "sl"
        elif h2 >= tp_f:
            exit_price = float(tp_f)
            exit_reason = "tp"
        else:
            exit_price = c2
        ...
        gross = (float(exit_price) - entry) * qty
        fee_in = qty * entry * TAKER_ENTRY_FEE
        fee_out = qty * float(exit_price) * exit_fee_r
        net_pnl = gross - fee_in - fee_out
        equity += net_pnl
...
def run_backtest(
    ...
    *,
    ...
    if st == "long_push_scalping":
        return _run_backtest_long_push_scalping(
            df,
            sp,
            trade_amount_usd=float(sp.get("tradeCapitalUsd", trade_amount_usd)),
            leverage=float(sp.get("leverage", leverage)),
